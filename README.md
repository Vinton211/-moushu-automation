...
小红书自动化发布系统是一个集成了Excel内容生成和小红书自动发布功能的Python应用程序。该系统能够自动为Excel中的标题生成内容，并将生成的内容发布到小红书平台，实现了从内容创作到发布的全流程自动化。

## 功能特性

### 1. Excel内容生成
- 支持从Excel文件读取标题
- 使用DeepSeek API自动生成符合小红书风格的内容
- 生成的内容限制在700字符以内
...
1. **小红书平台限制**：小红书平台可能会限制自动化操作，建议合理控制发布频率，避免账号被封。
...
6. **内容审核**：生成的内容可能需要人工审核，确保符合小红书平台的内容规范。

7. **图片上传**：如果需要上传图片，请确保图片路径正确，且图片格式符合小红书要求。
...
**注意**：本项目仅供学习和研究使用，请勿用于违反小红书平台规则的行为。使用本项目产生的一切后果由使用者自行承担。
